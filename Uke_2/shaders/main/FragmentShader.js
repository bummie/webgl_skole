...
const fragShader = `
    ...
    void main() 
    {
        float linValue = LinearizeDepth(texture2D(uTexture, vShadowCoord.xy).x);
        if (  texture2D(uTexture, vShadowCoord.xy).x < LinearizeDepth(vShadowCoord.z))//if ( vShadowCoord.z  < -0.88) //
        {
